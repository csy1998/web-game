...
var canvas = document.getElementsByTagName('canvas')[0], // 画布
    ctx = canvas.getContext('2d'), // 画布内容
    msg = document.getElementById('msg'),   // 历史消息框
    ranger = document.getElementById('ranger'), // 调节画笔线宽
    colors = document.getElementById('colors'); // 选择画笔颜色

var input = document.getElementById('input-msg'),   // 消息发送框
    users = document.getElementById('div-users'),   // 上场玩家/绘画玩家
    btnIn = document.getElementById('btn-in'),  // 上下场
    btnAutoin = document.getElementById('btn-autoin'), // 自动上场
    info = document.getElementById('info'), // 玩家信信息
    tops = document.getElementById('tops'); // 排行榜
    
tops.template = tops.querySelector('[role=template]').cloneNode(true);

info.time = info.querySelector('#time');    // 剩余时间 
info.player = info.querySelector('#player');    // 绘图玩家
info.word = info.querySelector('#word');    // 关键词语
